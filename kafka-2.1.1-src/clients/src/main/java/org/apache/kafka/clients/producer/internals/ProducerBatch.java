...
public final class ProducerBatch {
    ...
    public FutureRecordMetadata tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers, Callback callback, long now) {
        ...
        if (!recordsBuilder.hasRoomFor(timestamp, key, value, headers)) {
            ...
            //Thunk中的callback字段对应消息的callback对象，其另一个字段future是FutureRecordMetadata类型
    ...
    private void completeFutureAndFireCallbacks(long baseOffset, long logAppendTime, RuntimeException exception) {
        ...
        //标记整个ProducerBatch是“异常完成”还是“正常完成”
        produceFuture.set(baseOffset, logAppendTime, exception);

        // execute callbacks
        for (Thunk thunk : thunks) {//循环执行每个消息的Callback
            try {
                if (exception == null) {//正常处理完成
                    //将服务端返回的信息(offset和timestamp)和消息的其他信息封装成RecordMetadata
                    RecordMetadata metadata = thunk.future.value();
                    if (thunk.callback != null)//调用消息对应的自定义Callback
                        thunk.callback.onCompletion(metadata, null);
                } else {//处理过程中出现异常的情况，注意，第一个参数为null,与上面正常情况相反
