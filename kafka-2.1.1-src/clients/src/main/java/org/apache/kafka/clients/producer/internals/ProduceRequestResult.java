 ...
 */
public class ProduceRequestResult {//并没有实现java.util.concurrent.Future接口，但是其通过包含一个count值为1的CountDownLatch对象，实现了类似于Future的功能

    private final CountDownLatch latch = new CountDownLatch(1);
    private final TopicPartition topicPartition;

    //分区会为其中记录的消息分配一个offset，并通过此offset维护消息顺序。这个字段表示的是服务端为此RecordBatch中第一条消息分配的offset,
    //这样每条消息可以根据此offset以及自身在此RecordBatch中的相对偏移量，计算出其在服务端分区中的偏移量了。
     ...
     */
    public void done() {
        //当ProducerBatch中全部的消息被正常响应、或超时、或关闭生产者时，会调用ProduceRequestResult.done()方法，将
        //produceFuture标记为完成并通过ProduceRequestResult.error字段区分“异常完成”还是正常完成
        if (baseOffset == null)
