 ...
 *
 * 都是final类型的不可变性对象，是线程安全的，封装在Metadata对象中的
 *
 */
public final class Cluster {
    ...
    //Topic与PartitionInfo的映射关系，这里的List<PartitionInfo>中存放的分区必须是有Leader副本的Partition，而partitionsByTopic中记录的分区则不一定有Leader副本，因为某些中间状态，
    //例如Leader副本宕机而触发的选举过程中，分区不一定有Leader副本
    private final Map<String, List<PartitionInfo>> availablePartitionsByTopic;
    //记录了Node与PartitionInfo的映射关系，可以按照节点Id查询其上分布的全部分区的详细信息。
    private final Map<Integer, List<PartitionInfo>> partitionsByNode;
    //BrokerId与Node节点之间对应关系，方便按照BrokerId进行索引
