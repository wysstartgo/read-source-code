...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    public Future<RecordMetadata> send(ProducerRecord<K, V> record, Callback callback) {
        //每个消息都会有一个对应的callback对象作为回调
        //ProducerBatch中的Thunk中的callback字段就是指向对应消息的Callback对象
     ...
     *
     * 这里消息并不是真正地直接发送给Kafka，而是缓存在了RecordAccumulator中。之后，业务线程通过KafkaProducer.send()方法不断向
     * RecordAccumulator追加消息，当达到一定的条件，会唤醒Sender线程发送RecordAccumulator中的消息
     *
     */
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            try {
                //获取Kafka集群的信息，底层会唤醒Send线程更新Metadata中保存的Kafka集群元数据
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long maxWaitMs) throws InterruptedException {
        ...
        //KafkaProducer需要的集群元数据都保存在了Cluster这个类中
        Cluster cluster = metadata.fetch();

        if (cluster.invalidTopics().contains(topic))//检测Metadata中是否包含指定Toipc的元数据，若不包含，则将Topic添加到topics集合中，下次更新时会从服务端获取指定的Topic元数据
            throw new InvalidTopicException(topic);

        metadata.add(topic);

        Integer partitionsCount = cluster.partitionCountForTopic(topic);//尝试获取Topic中分区的详细信息，失败后会调用requestUpdate()方法设置Metadata.needUpdate字段，并得到当前元数据版本号
        ...
        do {
            ...
            sender.wakeup();//唤醒Sender线程，由Sender线程更新Metadata中保存的Kafka集群元数据。
            try {
                metadata.awaitUpdate(version, remainingWaitMs);//主线程调用awaitUpdate()方法，等待Sender线程完成更新
            ...
            cluster = metadata.fetch();//从Metadata中获取Topic分区的详细信息（即PartitionInfo集合）。若失败，则继续循环
    ...
    private int partition(ProducerRecord<K, V> record, byte[] serializedKey, byte[] serializedValue, Cluster cluster) {
        //这里优先根据ProducerRecord中partition字段指定的序号选择分区，如果ProducerRecord.partition字段没有明确指定分区编号，则通过
        //Partitioner.partition()方法选择Partition
