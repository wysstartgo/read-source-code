...
public final class RecordAccumulator {
    ...
    //指定每个ProducerBatch底层ByteBuffer的大小
    ...
    //Deque  这里是批量处理消息的地方，缓存了发往对应TopicPartition的消息
    //ProducerBatch中会通过MemoryRecordsBuilder构建MemoryRecords，而MemoryRecords才是消息最终存放的地方。
    //Deque是ArrayDeque类型，是非线程安全的集合，在追加消息或发送ProducerBatch的时候，需要加锁同步。每个Deque中都保存了发往对应TopicPartition的ProducerBatch
    private final ConcurrentMap<TopicPartition, Deque<ProducerBatch>> batches;
    //未发送完成的ProducerBatch集合，底层通过Set<ProducerBatch>集合实现
    private final IncompleteBatches incomplete;
    // The following variables are only accessed by the sender thread, so we don't need to protect them.
    private final Map<TopicPartition, Long> muted;
    //使用drain方法批量导出ProducerBatch时，为了防止饥饿，使用drainIndex记录上次发送停止时的位置，下次继续从此位置开始发送
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            synchronized (dq) {//对Deque加锁
                ...
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                //继续尝试追究加一次，这里加锁重试是为了防止多个线程并发向BufferPool申请空间后，造成内部碎片
                //线程1发现最后一个ProduceBatch空间不够用，申请空间并创建一个新ProduceBatch对象添加到Deque的尾部；线程2与线程1并发执行，也将新建一个
                //RecordBatch添加到Deque尾部。从上面的逻辑中我们可以得知，之后的追加操作只会在Deque尾部进行，这样就会出现上一个线程中的ProduceBatch1剩余部分不再被使用，所有
                //的消息都会追加到ProducerBatch2中去，而ProduceBatch2是在ProduceBatch1后面的，这就出现了内部碎片
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, headers, callback, dq);
                if (appendResult != null) {
                ...
                //在构建MemoryRecordsBuilder需要传入一个byteBuffer，而这个byteBuffer是通过BufferPool来构建的
                //追加失败，则使用上面申请的buffer创建ProducerBatch
                MemoryRecordsBuilder recordsBuilder = recordsBuilder(buffer, maxUsableMagic);
                ProducerBatch batch = new ProducerBatch(tp, recordsBuilder, time.milliseconds());
                //将Record追加到新建的ProducerBatch中，
                FutureRecordMetadata future = Utils.notNull(batch.tryAppend(timestamp, key, value, headers, callback, time.milliseconds()));
                //并将新建的ProduceBatch追加到对应的Deque尾部
                dq.addLast(batch);
                //将新建的ProduceBatch追加到incomplete集合
                incomplete.add(batch);

                // Don't deallocate this buffer in the finally block as it's being used in the record batch
                buffer = null;
                //返回RecordAppendResult，RecordAppendResult中的字段会作为唤醒Sender线程的条件。
                return new RecordAppendResult(future, dq.size() > 1 || batch.isFull(), true);
            }
        } finally {
            if (buffer != null)
                free.deallocate(buffer);//如果buffer不为null则释放上面申请的新空间
    ...
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers,
        ...
        //查找batches集合中对应队列的最后一个ProduceBatch对象
        ProducerBatch last = deque.peekLast();
        if (last != null) {
            //调用ProducerBatch的tryAppend方法完成消息的追加
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            ...
            Node leader = cluster.leaderFor(part);//查找分区的Leader副本所在的Node
            synchronized (deque) {
                //根据Cluster的信息检查Leader,Leader找不到，肯定不能发送信息
                if (leader == null && !deque.isEmpty()) {
                    ...
                    ProducerBatch batch = deque.peekFirst();//只取Deque中的第一个RecordBatch
                    if (batch != null) {
                        //通过计算得到下面五个条件
                        ...
                        if (sendable && !backingOff) {
                            //如果满足上面的五个条件，则将此Node信息记录到readyNodes集合中
        ...
        //遍历完成后返回ReadyCheckResult对象,其中记录了满足发送条件的Node集合、在遍历过程中是否有找不到Leader副本的分区(也可以认为是Metadata中当前的元数据过时了)
    ...
    private List<ProducerBatch> drainBatchesForOneNode(Cluster cluster, Node node, int maxSize, long now) {
        ...
        do {
            ...
            synchronized (deque) {
                ...
                ProducerBatch first = deque.peekFirst();//获取队列中第一个ProducerBatch
                if (first == null)
                    continue;
                ...
                if (size + first.estimatedSizeInBytes() > maxSize && !ready.isEmpty()) {
                    ...
                    //从队列中获取一个RecordBatch，并将这个RecordBatch放到ready集合中，每个TopicPartition只取一个RecordBatch
                    ...
                    //关闭压缩及底层输出流，并将MemoryRecords设置为只读
                    batch.close();
                    size += batch.records().sizeInBytes();
                    //每次只从每个队列中取出一个ProducerBatch放到ready集合中，这也是为了防止饥饿，提高系统的可用性
                    ready.add(batch);
    ...
    public Map<Integer, List<ProducerBatch>> drain(Cluster cluster, Set<Node> nodes, int maxSize, long now) {
        ...
        for (Node node : nodes) {
            //主要完成将TopicPartition到ProducerBatch集合的映射转换成了NodeId到ProducerBatch集合的映射
            //转换的原因是：在网络I/O层面，生产者是面向Node节点发送消息数据，它只建立到Node的连接并发送数据，并不关心这些数据属于哪个TopicPartition;
            //而在调用KafkaProducer的上层业务逻辑中，则是按照TopicPartition的方式产生数据，它只关心发送到哪个TopicPartition,并不关心这些TopicPartition
            //在哪个Node节点上。 Sender线程每次向每个Node节点至多发送一个ClientRequest请求，其中封装了追加到此Node节点上多个分区的消息，待请求到达服务端后，
            //由Kafka对请求进行解析
            List<ProducerBatch> ready = drainBatchesForOneNode(cluster, node, maxSize, now);//记录要发送的RecordBatch
    ...
    public void awaitFlushCompletion() throws InterruptedException {
        try {
            for (ProducerBatch batch : this.incomplete.copyAll())
                //这些线程通过ProduceRequestResult的await方法等待完成的状态，见ProduceBatch中的completeFutureAndFireCallbacks方法
